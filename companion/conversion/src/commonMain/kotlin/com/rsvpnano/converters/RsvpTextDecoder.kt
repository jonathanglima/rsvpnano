...
internal object RsvpTextDecoder {
    ...
    private fun decodeUtf16(data: ByteArray): String {
        if (data.size >= 2) {
            ...
            if (first == 0xfe && second == 0xff) return decodeUtf16BigEndian(data, offset = 2)
            if (first == 0xff && second == 0xfe) return decodeUtf16LittleEndian(data, offset = 2)
        }
        return decodeUtf16BigEndian(data)
    }

    private fun decodeUtf16LittleEndian(data: ByteArray, offset: Int = 0): String =
        decodeUtf16Units(data, offset) { index ->
            data[index].toUnsignedInt() or (data[index + 1].toUnsignedInt() shl 8)
        }

    private fun decodeUtf16BigEndian(data: ByteArray, offset: Int = 0): String =
        decodeUtf16Units(data, offset) { index ->
            (data[index].toUnsignedInt() shl 8) or data[index + 1].toUnsignedInt()
        }

    private fun decodeUtf16Units(data: ByteArray, offset: Int, unitAt: (Int) -> Int): String =
        buildString((data.size - offset) / 2) {
            var index = offset
