...
// in src/calibre/CalibreSyncPlan.h. JSON is hand-rolled with the
...
bool ensureLibraryDirectories() {
  // v0.0.9 moved the on-SD library from /books to /library and made
  // ensureDirectory return std::expected instead of taking a log tag.
  return StorageFiles::ensureDirectory(StoragePaths::kLibraryPath) &&
...
// Fold a Unicode codepoint to a single ASCII character for SD filenames.
//
// v0.0.9 removed src/text/LatinText.h (the old single-byte storage encoding)
// when the text stack moved to full Unicode, so the accent folding it used to
// provide lives here now. Only the Latin ranges matter: everything outside
// them returns '\0' and the caller substitutes '-'.
char foldCodepointToAscii(uint32_t codepoint) {
  if (codepoint < 0x80)
  ...
  static const char kLatinExtendedA[] =
      "AaAaAaCc" "CcCcCcDd" "DdEeEeEe" "EeEeGgGg"
      "GgGgHhHh" "IiIiIiIi" "IiIiJjKk" "kLlLlLlL"
      "lLlNnNnN" "nnNnOoOo" "OoOoRrRr" "RrSsSsSs"
      "SsTtTtTt" "UuUuUuUu" "UuUuWwYy" "YZzZzZzs";
  if (codepoint >= 0x100 && codepoint <= 0x17F)
    return kLatinExtendedA[codepoint - 0x100];
...
const char *CalibreSyncManager::manifestPath() {
  return "/library/.calibre-sync.json";
}

String CalibreSyncManager::sanitizeBaseName(const String &name) {
  String sanitized;
  sanitized.reserve(name.length());
  // Walk UTF-8 codepoints, folding accents to their ASCII base (é -> e, ó ->
  // o, ç -> c, ...) so filenames read as "Etica" rather than "--tica".
  // Anything that doesn't fold to a safe filename character becomes '-'.
  size_t i = 0;
  while (i < name.length()) {
    const uint32_t codepoint = decodeUtf8Codepoint(name, i);
    const char folded = foldCodepointToAscii(codepoint);
...
CalibreSyncManager::Result CalibreSyncManager::runSync(
  ...
  if (!net::connectStation(wifiSsid.c_str(), wifiPassword.c_str())) {
